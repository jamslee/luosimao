...
ies = ['_e5dn1i62u', '_kgyanf9aj', '_lenla0h9y']
...
def get_image(res,s):
    ...
    getImg = 'https://captcha.luosimao.com/api/frame?s=' + s + '&i='+random.choice(ies)+'&l=zh-cn'
...
def getToken(res,site_key):
    global ies
    geturl = 'https://captcha.luosimao.com/api/widget?k=' + site_key + '&l=zh-cn&s=normal&i='+random.choice(ies)
...
def run(yolo,site_key):
    try:
        config.Headers['User-Agent'] = random.choice(config.UserAgent)
        config.Headers['x-forwarded-for'] = str(random.randint(1, 255)) + '.' + str(random.randint(1, 255)) + '.' + str(
        ...
        type_all = []
        for k in r_image[1]:
            type_all.append(k['class'])
        if len(set(type_all)) == 3:
            back_loc = []
            ...
            r_image[0].save(os.path.join(save_img_dir,str(int(time.time()*1000))+'.png'))
    except:
        rep = {'code':'request exception'}
    return rep


if __name__ == '__main__':
    yolo = YOLO()
    # bbb = run(yolo,'95059ebc73a95c13d50ceb8e06939c94')
    for i in range(3):
        bbb = run(yolo,'95059ebc73a95c13d50ceb8e06939c94')
        print(bbb)